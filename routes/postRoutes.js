...
router.get("/", async (req, res) => {
  try {
    ...
    // Transform posts to handle both formats
    const transformedPosts = posts.map(post => {
...
// Update your upload-multiple route in postRoutes.js
router.post('/upload-multiple', upload.array('files', 10), async (req, res) => {
  try {
    const { caption, userId, locationName, locationCoordinates } = req.body;
    ...
    try {
      if (locationName) {
        ...
        // Manual location without coordinates
        if (!locationCoordinates && locationName.trim() !== "") {
          location = {
            name: locationName,
            coordinates: null
          };
...
router.post('/:postId/save', auth, async (req, res) => {
  try {
      ...
      // Send notification to post owner if saver is not the owner
      if (post.user._id.toString() !== userId) {
          // Create notification
          const notification = new Notification({
              userId: post.user._id,
...
router.post('/upload-multiple', upload.array('files', 10), async (req, res) => {
  try {
      ...
      // Upload each file to Cloudinary or your storage solution
